...
def train_single(subset):
    ...
    :return:
        mu: an array, the ith element means the expectation of the ith feature (by Maximum likelihood estimation)
...
def trains(samples, labels, c_list):
    ...
    :return:
        np.array(mu_m): a matrix, the ith row is the expectations of class c_list[i]'s class-conditional probability
    ...
    for c in c_list:
        ...
        subset = samples[idx, :]
        mu, sigma = train_single(subset)
        c_probs.append(c_prob)
        mu_m.append(mu)
        sigma_m.append(sigma)
    return np.array(mu_m), np.array(sigma_m), c_probs
...
def naive_bayes_predict(samples, c_list, mu_m, sigma_m, c_probs):
    ...
    :param mu_m: k * n matrix, k = len(c_list).
        mu_m[i][j] means the jth feature's expectation of c_list[i]'s class-conditional probability
